...
export async function runSavedCorrectionFlow({ evaluate, waitFor, record, button, set, read, revisit, mock, resetRequests, screenshot, command }) {
  await record('SAVED-CORRECTION-RECEIPT', async () => {
    ...
    try {
      ...
      await mock('/api/wordpress/connection-check',{user:{name:'QA receipt'},connector:{version:'1.3.5'}});
      await button('Collega WordPress','.proposal-remediation-slot');
      await waitFor("document.querySelector('.proposal-remediation-slot .wordpress-connection-control')?.textContent.includes('WordPress collegato')",'Verified WordPress connection through the real UI');
      ...
      await waitFor(`(async()=>{const m=await import('/src/remediationStore.js');const r=await m.readCorrection(${JSON.stringify(correctionId)});return Boolean(r?.after&&r?.appliedAt)})()`,'Applied correction is durably stored');
