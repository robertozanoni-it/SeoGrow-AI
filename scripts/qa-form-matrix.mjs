...
export async function runFormMatrix({ evaluate, waitFor, clickSidebar, reload, record, mode }) {
  ...
  const field = (scope, label) => `(() => { const root = document.querySelector(${q(scope)}); const labels = [...(root?.querySelectorAll('label') || [])]; const matches = labels.filter(l => (() => { const copy=l.cloneNode(true); copy.querySelectorAll('input,select,textarea').forEach(e=>e.remove()); return copy.textContent.trim(); })() === ${q(label)}); if (matches.length !== 1) throw new Error('Ambiguous/missing label: ' + ${q(label)}); return matches[0].control || matches[0].querySelector('input,select,textarea'); })()`;
