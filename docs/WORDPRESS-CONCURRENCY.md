...
- Le tassonomie restano fail-closed per gli storage non certificati. Dal Connector 1.3.10, Rank Math supporta CAS atomico esclusivamente per i campi scalari `title`, `meta_description` e `canonical` quando esiste una sola riga `wp_termmeta`: lock della tassonomia e della riga meta, confronto `BINARY`, UPDATE condizionale, rilettura pre-commit e prova di persistenza DB + `get_term_meta` post-commit. Riga assente/duplicata, `noindex`/robots e Yoast restano bloccati.
- Vecchio endpoint taxonomy-write resta disabilitato dopo validazione; il ciclo Doctor resta distinto. Apply e rollback passano dal writer atomico `/atomic-write`, con `STALE_CONFLICT` su snapshot cambiato e `ATOMIC_RESULT_UNVERIFIED` quando l'esito del commit o la persistenza finale non sono dimostrabili.
