...
const workflow = await readFile(new URL("../.github/workflows/connector-package.yml", import.meta.url), "utf8");
...
test("il link esterno 404 è mostrato in un campo dedicato con Apri e Copia", () => {
  ...
  assert.match(ui, /evidenceBrokenTarget/);
  assert.match(css, /\.seogrow-broken-link-target-field/);
  assert.match(css, /\.seogrow-broken-link-target-row/);
});

test("il pacchetto Connector aggiornato è distinguibile come 1.3.10", () => {
  assert.match(connector, /Version:\s*1\.3\.10/);
  assert.match(connector, /elementor-link-cleanup-write\.php/);
  assert.match(connector, /elementor-shared-link-remediation\.php/);
  assert.match(workflow, /seogrow-connector-1\.3\.10/);
  assert.match(workflow, /artifacts\/seogrow-connector-1\.3\.10\.zip/);
});
