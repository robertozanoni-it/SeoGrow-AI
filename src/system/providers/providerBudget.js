import { observedNumber } from "../../modules/audit/data.js";

export const budgetMoney = (value, digits = 2) =>
  observedNumber(value) === null ? "—" : `$${Number(value).toFixed(digits)}`;
...
export function providerBudgetHealth(status = {}, config = {}) {
  ...
  if (!configured) return {
    ...
    spent, reserved, budget: validBudget ? budget : null,
    remaining: validBudget && budget > 0 && committed !== null ? Math.max(0, budget - committed) : null,
    percent: 0, explicit,
  };
  ...
  if (remaining <= 0) return {
    tone: "danger", label: "Budget esaurito", detail: "Le nuove richieste a pagamento vengono bloccate.",
    spent, reserved, budget, remaining, percent, explicit,
  };
  if (percent >= 80) return {
    tone: "warning", label: "Budget quasi esaurito", detail: `Rimane ${money(remaining)} prima del limite mensile.`,
    spent, reserved, budget, remaining, percent, explicit,
  };
  return {
    tone: "ok", label: "Budget disponibile", detail: `Rimangono ${money(remaining)} nel limite mensile.`,
    spent, reserved, budget, remaining, percent, explicit,
  };
}
