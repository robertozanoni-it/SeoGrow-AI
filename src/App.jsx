...
function RankingsPage({
  ...
  const run = async (event) => {
    ...
    try {
      ...
      const data = await response.json().catch(() => null);
      if (!response.ok)
        throw new Error(data?.error || `Controllo posizioni non riuscito (HTTP ${response.status})`);
      if (!data || typeof data !== "object" || !Array.isArray(data.rankings))
        throw new Error("Controllo posizioni non riuscito: risposta non valida.");
      if (data.monthlyCost != null) onUsage?.(data.monthlyCost);
      onSave(data);
      if (data.partial)
        setError(
