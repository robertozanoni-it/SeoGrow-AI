        ...
        <section className="reference-client-grid">
          {filtered.map((c) => {
            ...
            return <article className="reference-client-card client-card" key={c.id} onClick={(event) => { if (!event.target.closest("a,button")) openClient(event, c.id); }}>
              ...
              <footer><span>{dataset ? `${dataset.queries.length} query importate` : "Dati Search Console non disponibili"}</span><div><button className="primary" onClick={(event) => openClient(event, c.id)}>Apri scheda →</button><button className="secondary mini" onClick={(event) => { event.stopPropagation(); onDownloadReport(c.id); }}><Download /></button><button className="secondary mini" aria-label={`Modifica ${c.name}`} onClick={(event) => { event.stopPropagation(); setEditingId(c.id); setForm({ name: c.name, url: c.url }); setFormError(""); setOpen(true); }}>•••</button><button className="secondary mini danger-text" aria-label={`Elimina ${c.name}`} onClick={(event) => { event.stopPropagation(); onDeleteClient(c.id); }}><Trash2 /></button></div></footer>
            </article>;
          })}
        </section>
