...
test('policy pages excluded, editorial privacy articles retained', () => {
  ...
  assert.equal(data.legalScopeVersion, 4);
});
...
test('crawl failures and link rows sourced only from legal pages are excluded', () => {
  const data = excludeLegalSeo({
    url: 'https://example.com/',
    failures: [
      { url: 'https://example.com/privacy-policy/', reason: 'Timeout' },
      { url: 'https://example.com/corsi/', reason: 'Timeout' },
    ],
    brokenLinks: [
      { url: 'https://example.com/missing-legal', status: 404, sources: ['https://example.com/privacy-policy/'] },
      { url: 'https://example.com/missing-content', status: 404, sources: ['https://example.com/corsi/'] },
    ],
  });
  assert.deepEqual(data.failures.map(item => item.url), ['https://example.com/corsi/']);
  assert.deepEqual(data.brokenLinks.map(item => item.url), ['https://example.com/missing-content']);
});
