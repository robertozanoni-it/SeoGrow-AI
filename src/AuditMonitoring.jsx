...
export function ProjectMonitoring({ client, settings, onSave }) {
  ...
  return <section className="panel planning-panel project-monitoring"><span className="workflow-category">Controlli nel tempo · facoltativi</span><h2>Monitoraggio e aggiornamento dati</h2><ol className="workflow-instructions"><li>Scegli dopo quanti giorni vuoi ricevere gli avvisi sui dati.</li><li>Se desideri controlli ricorrenti, abilita l’audit periodico e scegli la frequenza.</li><li>Leggi l’esito e le variazioni qui sotto. Per correggere i problemi torna ad “Analizza e correggi”.</li></ol><label className="report-option"><input type="checkbox" checked={settings.freshnessEnabled !== false} onChange={event => onSave((current) => ({ ...current, freshnessEnabled: event.target.checked }))} />Mostra avvisi sui dati mancanti o non aggiornati</label><label>Soglia avvisi<select value={settings.freshnessDays || 7} onChange={event => onSave((current) => ({ ...current, freshnessDays: Number(event.target.value) }))}>{[1, 3, 7, 14, 30, 90].map(days => <option key={days} value={days}>{days} giorni</option>)}</select></label>
    ...
    <label className="report-option"><input type="checkbox" disabled={!navigator.locks} checked={config.enabled} onChange={event => onSave((current) => ({ ...current, monitor: { ...monitorConfig(current.monitor), enabled: event.target.checked } }))} />Abilita audit periodico per {client.name}</label><label>Frequenza<select value={config.hours} onChange={event => onSave((current) => ({ ...current, monitor: { ...monitorConfig(current.monitor), hours: Number(event.target.value) } }))}>{[6, 12, 24, 168].map(hours => <option key={hours} value={hours}>Ogni {hours} ore</option>)}</select></label>
