...
export { suggestPageForQuery } from "./modules/rank/pageSuggestion.js";
...
export function buildClientReport({
  ...
  const queryRows = (dataset?.queries || [])
    ...
    .join("");
  const rankingRows = (rankings?.items || rankings?.rankings || []).slice(0, 500).map((item) => `<tr><td>${escapeHtml(item.keyword)}</td><td>${escapeHtml(item.position ?? "—")}</td><td>${escapeHtml(item.delta ?? "—")}</td><td>${reportLink(item.url, item.url)}</td></tr>`).join("");
  const editorialRows = (Array.isArray(editorial) ? editorial : []).slice(0, 500).map((item) => `<tr><td>${escapeHtml(item.title)}</td><td>${escapeHtml(item.type || "—")}</td><td>${escapeHtml(item.date || "—")}</td><td>${reportLink(item.url, item.url)}</td></tr>`).join("");
  ...
  const headings = { overview: "Sintesi executive", tasks: "Task del progetto", issues: "Problemi tecnici verificati", geo: "Preparazione GEO", queries: "Top query Search Console", rankings: "Posizionamenti", editorial: "Piano editoriale", links: "Link interni ed esterni non raggiungibili" };
