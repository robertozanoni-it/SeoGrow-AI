...
test("migration preserves native data; stale tab writes cannot corrupt the restored generation", async () => {
  ...
  const { initializeWorkspace, workspaceStorage, flushWorkspace, openWorkspaceDb, readWorkspace, commitWorkspaceRestore } = await import(`./workspaceDatabase.js?migration-${Date.now()}`);
