...
test("Connector carica un purge mirato cache-only per tassonomie Rank Math", () => {
  ...
  assert.match(purgeModule, /taxonomy-public-cache-purge/);
...
test("Rank Math general usa il purge mirato solo nel caso frontend-only e verifica dopo il purge", () => {
  ...
  const purge = general.indexOf("taxonomy-public-cache-purge");
  const verify = general.indexOf("RANK_MATH_GENERAL=PUBLIC_CACHE_RECOVERED");
  const writeCycle = general.indexOf("single controlled apply/verify/rollback cycle");
