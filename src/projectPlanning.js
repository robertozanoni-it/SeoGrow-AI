...
export function planItems(plan, saved) {
  const entries = Array.isArray(saved) ? saved.filter(item => typeof item?.id === "string" && typeof item.title === "string" && validDate(item.date)).slice(0, 200) : [];
  const all = new Map(entries.map(item => [item.id, item]));
  for (const item of (Array.isArray(plan) ? plan : [])) if (typeof item?.id === "string" && typeof item.title === "string") all.set(item.id, { ...item, date: all.get(item.id)?.date || "" });
  return [...all.values()];
}
export function scheduleItem(saved, item, date) {
  if (typeof item?.id !== "string" || typeof item.title !== "string") throw new Error("Attività non valida");
  if (date && !validDate(date)) throw new Error("Data non valida");
  const others = planItems([], saved).filter(entry => entry.id !== item.id);
