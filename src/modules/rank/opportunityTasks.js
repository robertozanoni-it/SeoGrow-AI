...
export function opportunityTask(row, dataset, tab = "quickWins") {
  ...
  const relation = dataset?.queryPages?.find(
    (item) => normalizedQuery(item.dimension || item.query) === normalizedQuery(query),
  );
...
export function findExistingTask(tasks, values, clientId) {
  return tasks.find((item) => {
    if (
      ...
      normalizeClientId(item.sourceClientId) !== normalizeClientId(clientId) ||
      item.stale ||
    ...
    if (
      ...
      item.kind === "search" &&
      values.query &&
      normalizedQuery(item.query) === normalizedQuery(values.query)
    )
      return true;
    ...
    return (
      ...
      item.kind === "manual" &&
      !item.sourceUrl &&
      Boolean(values.sourceUrl) &&
      item.targetUrl === values.sourceUrl &&
      !values.targetUrl
    );
  });
}
