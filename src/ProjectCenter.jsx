...
export default function ProjectCenter({ client, dataset, analysis, connection, aiConfigured, settings = {}, onSave, onNavigate, onReport, children }) {
  ...
  return <><div className="page-title"><div><h1>Centro progetto — {client.name}</h1><p>Configura il lavoro e controlla quali dati sono disponibili.</p></div></div>
    <section className="panel planning-panel project-setup"><span className="workflow-category">Preparazione del progetto</span><h2>Configurazione guidata</h2><p>Da completare all’inizio o quando cambiano i dati del progetto. Segui le quattro schede; poi passa ad “Analizza e correggi”. Le impostazioni vengono salvate mentre scrivi.</p><ol className="wizard-steps">{["Obiettivo", "Dati SEO", "WordPress", "Riepilogo"].map((label, index) => <li key={label}><button className="secondary" aria-current={index === step ? "step" : undefined} onClick={() => setStep(index)}>{index + 1}. {label}</button></li>)}</ol>
      {step === 0 && <label>Obiettivo del progetto<small>Scrivi il risultato che vuoi ottenere, poi premi Avanti.</small><textarea maxLength={1000} value={objective} onChange={event => onSave({ ...settings, objective: event.target.value })} placeholder="Es. aumentare le richieste per i corsi di yoga" /></label>}
      {step === 1 && <><p>Importa i dati Search Console per analizzare le query ed esegui l’audit per rilevare i problemi delle pagine. Poi torna qui e premi Avanti.</p><p>Search Console: {dataset ? "dati importati" : "dati mancanti"}. Audit del sito: {analysis ? "disponibile" : "non eseguito"}.</p><button className="secondary" onClick={() => onNavigate("Integrazioni")}>Importa Search Console</button><button className="secondary" onClick={() => onNavigate("Audit SEO")}>Apri audit SEO</button></>}
      {step === 2 && <><p>Controlla lo stato qui sotto. Se la connessione è già verificata puoi proseguire; altrimenti collegala nelle Integrazioni o nel passaggio 3 delle correzioni.</p><p>WordPress: {verified ? "connessione verificata in questa sessione" : "verifica assente o scaduta"}. La verifica della connessione legge il sito senza modificare contenuti.</p><button className="secondary" onClick={() => onNavigate("Integrazioni")}>Verifica connessione WordPress</button></>}
