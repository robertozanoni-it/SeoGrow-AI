...
test("a late Google status response preserves already loaded properties", () => {
  ...
  const merged = mergeGoogleStatus(
    { configured: true, connected: true, properties },
    { configured: false, connected: false, redirectUri: "http://localhost:8787/api/google/callback" },
  );
  assert.deepEqual(merged.properties, properties);
  assert.equal(merged.connected, true);
  assert.equal(merged.configured, true);
  assert.ok(merged.redirectUri);
});
...
test("legacy Google integration exports remain referentially identical", () => {
  assert.equal(mergeGoogleStatus, legacyMergeGoogleStatus);
  assert.equal(normalizeGoogleProperties, legacyNormalizeGoogleProperties);
});
