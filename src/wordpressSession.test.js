...
test("WordPress sessions are isolated by project and installation and expire", () => {
  ...
  rememberWordPressSession(1, c);
  forgetWordPressSession(1, c.url);
  assert.equal(getWordPressSession(1, c.url), null);
});

test("System and legacy WordPress session exports remain referentially identical", () => {
  assert.equal(getWordPressSession, legacyGetWordPressSession);
  assert.equal(rememberWordPressSession, legacyRememberWordPressSession);
  assert.equal(forgetWordPressSession, legacyForgetWordPressSession);
});
...
test("Shared ownership failure is explained as blocked, never a proposed change", () => {
  ...
  assert.match(p.title, /bloccata/);
  assert.match(p.title, /nessuna proposta/);
...
test("Human readable before/after preserves exact values including literal markup", () => {
  assert.deepEqual(
    readableCorrectionFields({
      data: {
        changed: ["meta.rank_math_description"],
        previewBefore: { meta: { rank_math_description: "" } },
        previewAfter: { meta: { rank_math_description: "<b>Test</b>" } },
      },
    }),
    [{ field: "meta.rank_math_description", label: "Meta description", before: "(vuoto)", after: "<b>Test</b>" }],
  );
});
