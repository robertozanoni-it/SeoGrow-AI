...
function mockWp(t, update = { id: 42, title: { raw: 'before' } }) {
 ...
 t.mock.method(globalThis, 'fetch', async (url, init) => { calls.push({ url: String(url), init }); return Response.json(init.method === 'POST' ? update : { id: 42, title: { raw: 'after' }, content: { raw: 'someone else changed this' } }); });
 return calls;
}
...
test('rollback cannot report success when WordPress ignores the requested value', async t => {
 ...
 const result = await invoke(body); assert.notEqual(result.data.ok, true); assert.equal(result.data.code, 'ROLLBACK_UNVERIFIED');
});
