...
const quickGoals = [
  "Trova le 10 migliori opportunità SEO",
  "Perché il traffico organico è diminuito?",
  "Quali pagine posso portare in Top 10?",
  "Quali contenuti devo aggiornare?",
  "Trova opportunità di internal linking",
];

const toolLabels = {
  "data.gsc": ["Dati Search Console", "Dataset salvato del progetto"],
  "data.analysis": ["Audit SEO", "Ultima analisi tecnica salvata"],
  "data.rankings": ["Ranking DataForSEO", "Storico posizionamenti salvato"],
  "seo.opportunities": ["Calcolo opportunità", "Motore opportunità SeoGrow"],
  "seo.trafficDrop": ["Analisi calo traffico", "Confronto periodi Search Console"],
  "seo.contentDecay": ["Analisi content decay", "Cali e piano contenuti"],
  "seo.internalLinks": ["Suggerimenti link interni", "Risultati del crawl salvato"],
};
...
const problemPrompt = (detail) => {
  ...
  return [
    ...
    "Usa solo dati verificabili del progetto, distingui osservazioni da inferenze e non dichiarare risolto il problema: le azioni operative appartengono a Correzioni e Task.",
  ].filter(Boolean).join(" ");
};
...
export default function AgentPage({ client, dataset, analysis, rankings, savedRuns = [], onSaveRun, onDeleteRun, onCreateTask }) {
  ...
  const runtime = useMemo(() => {
    ...
    return {
      ...
      orchestrator: new SeoAgentOrchestrator({ registry, onUpdate: setCurrentRun }),
    };
  }, []);
  ...
  useEffect(() => {
    const refresh = () => setCanonicalRevision((value) => value + 1);
    for (const event of ["storage", "seogrow-storage-ok", "seogrow-remediation-history", "seogrow-task-cause-reconciled", "seogrow-tasks-changed"]) window.addEventListener(event, refresh);
    return () => {
      for (const event of ["storage", "seogrow-storage-ok", "seogrow-remediation-history", "seogrow-task-cause-reconciled", "seogrow-tasks-changed"]) window.removeEventListener(event, refresh);
    };
  }, []);
  ...
  const canonicalTasks = useMemo(() => {
    canonicalRevision;
    return readWorkspaceJson(TASKS_KEY, []).filter((task) => Number(task?.sourceClientId) === Number(client.id));
  }, [canonicalRevision, client.id]);
  const canonicalCorrections = useMemo(() => {
    canonicalRevision;
    return remediationIndex().filter((row) => Number(row?.clientId) === Number(client.id));
  }, [canonicalRevision, client.id]);
  ...
  const persistRun = (value) => {
    const logged = asAgentAnalysisLog(value);
    setCurrentRun(logged);
    onSaveRun(logged);
    return logged;
  };
  const logAction = (baseRun, entry) => {
    if (!baseRun?.id) return baseRun;
    const next = appendAgentLog(baseRun, entry);
    setCurrentRun(next);
    onSaveRun(next);
    return next;
  };
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (raw) {
        ...
        if (applyPrefill(detail)) {
          sessionStorage.removeItem(AGENT_PREFILL_KEY);
          autorunPending.current = sessionStorage.getItem(AGENT_AUTORUN_KEY) === "1";
          sessionStorage.removeItem(AGENT_AUTORUN_KEY);
        }
      }
    } catch {
      sessionStorage.removeItem(AGENT_PREFILL_KEY);
  ...
  const start = async () => {
    ...
    if (!capabilityGate.ok) {
      setActionError(capabilityGate.errors.join(" "));
      return;
    }
    ...
    try {
      ...
      if (contextualProblem && problemNeedsFreshAudit(analysis, contextualProblem)) {
        const confirmation = await runConfirmationAudit({
          clientId: client.id,
          siteUrl: client.url,
          sourceUrl: contextualProblem.sourceUrl || contextualProblem.url,
          issueType: contextualProblem.issueType,
          issueLabel: contextualProblem.title || contextualProblem.issueLabel,
        ...
        result = {
          ...
          observations: [...(result.observations || []), {
            id: `fresh-audit-${Date.now()}`,
            tool: "data.analysis",
            status: "COMPLETED",
            usable: true,
            result: { data: confirmation.audit, source: "LIVE_AUDIT", freshness: "fresh", observedAt: new Date().toISOString(), durationMs: 0, estimatedCost: 0, actualCost: 0 },
          }],
          resolutionOutcome: confirmation.covered && !confirmation.stillPresent
            ? { kind: "analysis-only", note: "La verifica recente non rileva più il finding. SEO Agent non chiude il problema né le Task: conferma lo stato nel modulo Audit/Correzioni, che resta la source of truth." }
            : { kind: "analysis-only", note: confirmation.note || "Verifica recente completata. Le eventuali azioni restano nei moduli proprietari." },
        };
      } else {
        result = contextualProblem
          ? buildProblemAgentRun({ goal, detail: contextualProblem, analysis, projectId: client.id })
          : await orchestrator.run(goal, input);
      }
      const logged = appendAgentLog(asAgentAnalysisLog(result), { phase: "analysis", kind: "ANALYSIS_COMPLETED", label: "Analisi conclusa con capability reali del progetto" });
      persistRun(logged);
    } catch (error) {
      setActionError(error?.message || "Non è stato possibile avviare l’analisi.");
    } finally {
      operationLock.current = false; setRunning(false);
    }
  };
  ...
  const saveRecommendationTask = (item) => {
    ...
    logAction(run, {
      phase: "action",
      kind: "TASK_CREATED",
      label: `Task creata: ${task.title}`,
      targetType: "task",
      targetId: task.id,
      problemKey: problemContext?.issueKey || item?.issueKey || "",
      sourceUrl: item.page || "",
    });
    return task;
  };
  ...
  const runProblemAction = (item) => {
    ...
    if (priority.mode === "automatic") {
      ...
      const message = priority.kind === "canonical"
        ? `Confermi che ${problem.sourceUrl} debba essere la versione canonica pubblica? SeoGrow aprirà Correzioni e preparerà il Prima/Dopo; nessuna scrittura parte da SEO Agent.`
        : `Confermi che ${problem.sourceUrl} debba essere indicizzabile? SeoGrow aprirà Correzioni e preparerà il Prima/Dopo; nessuna scrittura parte da SEO Agent.`;
    ...
    if (!opened) {
      setActionError("Non è stato possibile aprire il percorso di risoluzione. Riapri il finding da Problemi e riprova.");
      return false;
    }
    logAction(run, {
      phase: "action",
      kind: "CORRECTION_HANDOFF",
      label: `Handoff a Correzioni: ${priority.label}`,
      targetType: "correction",
      problemKey: problem.key || problemContext?.issueKey || item?.issueKey || "",
  ...
  return <div className="reference-agent-page">
    <section className="reference-agent-hero">
      <div className="reference-agent-heading"><span><Sparkles /></span><div><h1>SEO Agent</h1><p>Analizza {client.name} usando soltanto capability realmente disponibili; le modifiche restano nei moduli proprietari.</p></div></div>
      <div className="reference-agent-hero-copy"><strong>Analisi → proposta → azione.</strong><span><ShieldCheck /> Nessuna scrittura diretta</span></div>
    </section>
    ...
    <section className="reference-agent-command-card">
      <div><h2>Capability disponibili</h2><p>L’Agent esegue solo tool di lettura realmente registrati nel progetto.</p></div>
      <div className="reference-agent-shortcuts">{capabilityGate.capabilities.map((item) => { const [label] = toolLabels[item.name] || [item.name]; return <span key={item.name}><CheckCircle2 /> {label}</span>; })}</div>
    </section>
    <section className="panel agent-console">
      <div className="agent-form-field"><label>Modalità operativa</label><p className="agent-help"><ShieldCheck /> Sola lettura. SEO Agent non possiede writer e non applica modifiche.</p></div>
      <div className="agent-form-field"><label htmlFor="seo-agent-goal">Cosa vuoi analizzare?</label><textarea id="seo-agent-goal" disabled={running} rows="3" value={goal} onChange={(event) => { setGoal(event.target.value); setProblemContext(null); }} placeholder="Es. Trova le 10 migliori opportunità SEO" /></div>
      ...
      {running && <p className="agent-help" role="status">Analisi in corso. Nessuna azione di scrittura può essere eseguita da questa fase.</p>}
      {!running && !goal.trim() && <p className="agent-help">Scegli un obiettivo supportato oppure descrivi il risultato SEO che vuoi analizzare.</p>}
    ...
    {run && <section className="panel agent-run" aria-live="polite">
      <div className="panel-head"><div><h2>1 · Analisi</h2><p>{run.goal}</p></div><span className={`priority ${run.status === AgentStatus.COMPLETED ? "bassa" : "media"}`}>{agentRunDisplayStatus(run)}</span></div>
      <ol className="agent-steps">{(run.plan?.steps || []).map((step) => { const observation = (run.observations || []).findLast((item) => item.tool === step.tool); const [label, description] = toolLabels[step.tool] || [step.tool, "Capability registrata"]; return <li key={step.id} className={`agent-step-${String(step.status || "pending").toLowerCase()}`}>{["COMPLETED", "CACHED"].includes(step.status) ? <CheckCircle2 aria-hidden="true" /> : <Circle aria-hidden="true" />}<div><strong>{label}</strong><small>{description}</small><span>Stato: {agentStatusLabel(step.status || "PENDING")} · Fonte: {observation?.result?.source || "—"} · Aggiornamento dati: {observation?.result?.freshness || "—"} · Durata: {observation?.result?.durationMs ?? "—"} ms · Costo: {agentCostLabel(observation?.result)}</span>{observation?.error && <em>{observation.error}</em>}</div></li>; })}</ol>
    ...
    {run?.recommendations?.length > 0 && <section className="agent-recommendations"><h2>2 · Proposte</h2>{run.recommendations.map((item) => {
      ...
      return <article className="panel agent-recommendation" key={item.id}>
        ...
        <p><strong>Interpretazione:</strong> {item.interpretation}</p><p><strong>Proposta:</strong> {item.recommendation}</p>
        <div className="agent-recommendation-footer"><small>Confidenza {item.confidence ?? "—"}% · Fonti: {Array.isArray(item.sources) && item.sources.length ? item.sources.join(", ") : "non disponibili"}</small></div>
        <div className="agent-controls" aria-label="3 · Azione">
          <strong>3 · Azione</strong>
          {isProblem ? <><button className="primary" disabled={running} onClick={() => runProblemAction(item)}><Sparkles /> {priority.label}</button><button className="secondary" disabled={running} onClick={() => saveRecommendationTask(item)}>Salva come task</button></> : <button className="secondary" disabled={running} onClick={() => saveRecommendationTask(item)}>Crea task</button>}
        </div>
      </article>;
    })}</section>}
